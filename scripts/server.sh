...
LOGTRACE_OPTS=$LOGTRACE_BIN/runtime-s.properties
...
if [ -z "$LOGTRACE_OUT" ]; then
  LOGTRACE_OUT="$LOGTRACE_LOG"/server.out
fi
...
cd $LOGTRACE_HOME
exec java $JAVA_OPTS -cp $CLASSPATH  com.c1000k.netty.server.Netty5C1000kServer "172.18.2.127" "8082" >> "$LOGTRACE_OUT" 2>&1 &
